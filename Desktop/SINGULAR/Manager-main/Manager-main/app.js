...
function bindListeners(){ if(__bound) return; __bound=true;
  ...
  try {
    ...
    localStorage.setItem('obraId', state.selectedObraId || '');
    // Reload dependent UI
    await loadInventario();
    renderObraInfo();
  } catch (err) {
    console.error('obraSelect change error', err);
    toast('error','No se pudo cambiar de obra');
  }
}); renderObrasSelect(); await loadInventario(); renderObraInfo(); });
...
function collectRowEdits(tr, original){
  ...
  const __oid = get('select[data-f="obra_id"]').value; if (__oid) payload.obra_id = __oid; return payload;
}
...
function renderCatResults(items, { obra, cat, subt, brand }){
  ...
  ${arr.map(x => `<div class="list-compact">${escapeHtml(x.nombre)} - ${escapeHtml(x.subtipo||'—')} - ${escapeHtml(x.marca||'—')}</div>`).join('')}
</div>
      </div>
    `;
...
// [ANCHOR: ADD_FORM_AUTOFILL_NAME_HANDLER]
(function attachAddFormAutofill(){
  const nombre = document.getElementById('itemNombre');
  const categoria = document.getElementById('itemCategoria');
  const subtipo = document.getElementById('itemSubtipo');
  const badge = document.getElementById('autofillBadge');
  const powerWrap = document.getElementById('powerSourceWrap');

  let t = null;
  function onInput(){
    clearTimeout(t);
    t = setTimeout(async ()=>{
      const norm = normalizeName(nombre.value||'');
      if (!norm || norm.length < 2) { badge?.classList.add('hidden'); return; }
      try{
        const res = await fetch('/.netlify/functions/item-templates?name='+encodeURIComponent(norm));
        const tpl = await res.json();
        if (tpl && tpl.default_category){
          if (categoria) categoria.value = tpl.default_category;
          if (subtipo && tpl.default_subtype != null){
            subtipo.value = tpl.default_subtype || '';
            // mostrar/ocultar power source segun subtipo
            powerWrap?.classList.toggle('hidden', subtipo.value !== 'electrica');
          }
...
// Autoupsert de plantilla al enviar el formulario
(function attachTemplateUpsertOnSubmit(){
  ...
  form?.addEventListener('submit', ()=>{
    try{
      ...
      if (nombre && categoria){
        fetch('/.netlify/functions/item-templates/upsert',{
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body: JSON.stringify({ name: nombre, category: categoria, subtype: subtipo })
        });
      }
    }catch(e){}
  }, { capture:true });
...
// Carga de marcas para dropdown
(async function loadBrandsDropdown(){
  const sel = document.getElementById('itemMarca');
  if (!sel) return;
  try{
    const res = await fetch('/.netlify/functions/brands');
    const brands = await res.json();
    const options = ['<option value="">—</option>','<option value="__new__">Nueva marca…</option>']
      .concat(brands.map(b=>`<option value="${b.id}">${b.name}</option>`));
...
// Precios: pivot
function initPrecioView(){
  const tipoSel = document.getElementById('precioTipo');
  const famSel = document.getElementById('precioFamilia');
  async function load(){
    const type = tipoSel.value;
    const family = famSel.value;
    const res = await fetch('/.netlify/functions/prices/pivot?type='+encodeURIComponent(type)+'&family='+encodeURIComponent(family));
    const data = await res.json();
    renderPivot(data);
  }
  tipoSel?.addEventListener('change', load);
  famSel?.addEventListener('change', load);
  load();
}
function renderPivot({suppliers, items}){
  const thead = document.getElementById('pivotHead');
  const tbody = document.getElementById('pivotBody');
  if (!thead || !tbody) return;
  thead.innerHTML=''; tbody.innerHTML='';
  const htr = document.createElement('tr');
  htr.innerHTML = '<th aria-sort="none">Ítem</th>' + suppliers.map(s=>`<th title="${s.name}">${s.name}</th>`).join('');
  thead.appendChild(htr);
  const frag = document.createDocumentFragment();
  for (const it of items){
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${(it.item_name||'').replace(/</g,'&lt;')}</td>` + suppliers.map(s=>{
      const v = it['s_'+s.id];
...
function initDatosView(){
  const subtipoSel = document.getElementById('datosSubtipo');
  const powerWrap = document.getElementById('datosPowerWrap');
  const powerSel = document.getElementById('datosPower');
  function updatePower(){ powerWrap.classList.toggle('hidden', subtipoSel.value!=='electrica'); }
  subtipoSel?.addEventListener('change', updatePower); updatePower();

  document.getElementById('exportPdfBtn')?.addEventListener('click', async ()=>{
    const tipo = document.getElementById('datosTipo').value;
    const obra = document.getElementById('datosObra').value;
    const familia = document.getElementById('datosFamilia').value;
    const subtipo = subtipoSel.value;
    const power = powerSel.value;

    let url = '/.netlify/functions/items?tipo='+tipo;
    if (obra && obra!=='all') url += '&obraId='+obra;
    const res = await fetch(url);
    const items = await res.json();
    const list = (Array.isArray(items)?items:[]).filter(x=>{
      if (familia!=='todas' && x.family!==familia) return false;
      if (subtipo!=='all' && x.subtipo!==subtipo) return false;
      if (subtipo==='electrica' && power!=='all' && x.power_source!==power) return false;
      return true;
    });

    const { jsPDF } = window.jspdf;
    const doc = new jsPDF('p','pt');
    const title = 'Listado '+(tipo==='tool'?'herramientas':'materiales');
    const date = new Date().toLocaleString('es-ES', { hour12:false, timeZone:'Europe/Madrid' });
    doc.text(title, 40, 36);
    doc.text(date, 40, 54);
    const rows = list.map(it=>[ it.name||it.nombre||'', it.category||it.categoria||'', it.project_name||it.obra||'', it.brand_name||it.marca||'', it.subtipo||'', it.power_source||'' ]);
    doc.autoTable({ startY: 72, head:[['Nombre','Categoría','Obra','Marca','Subtipo','Energía']], body: rows });
    const y = (doc).lastAutoTable.finalY + 24;
    doc.text('Total de ítems: '+rows.length, 40, y);
    doc.save('export.pdf');
  });
}

// Trabajadores: vista semanal
function initTrabajadoresView(){
  const weekStart = document.getElementById('weekStart');
  const list = document.getElementById('trabajadoresList');
  function monday(d){ const dt=new Date(d); const day=(dt.getDay()+6)%7; dt.setDate(dt.getDate()-day); return dt; }
  function fmt(d){ return d.toISOString().slice(0,10); }
  const now = new Date(); if (weekStart) weekStart.value = fmt(monday(now));

  async function load(){
    const start = weekStart.value; if (!start) return;
    await fetch('/.netlify/functions/workers/seed-week', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ start }) });
    const res = await fetch('/.netlify/functions/workers/week?start='+encodeURIComponent(start));
    const data = await res.json();
    renderWeek(data);
  }
  weekStart?.addEventListener('change', load);
  document.getElementById('seedWeekBtn')?.addEventListener('click', load);
  load();

  function renderWeek(data){
    const days = data.days; const wk = data.workers;
    const wrap = document.createElement('div'); wrap.className='table-wrap';
    const table = document.createElement('table'); table.className='table'; wrap.appendChild(table);
    const thead = document.createElement('thead'); const htr=document.createElement('tr');
    htr.innerHTML = '<th>Trabajador</th>' + days.map(d=>`<th>${d}</th>`).join('') + '<th>Total</th>';
    thead.appendChild(htr); table.appendChild(thead);
    const tbody = document.createElement('tbody');
    for (const row of wk){
      const tr = document.createElement('tr');
      tr.innerHTML = `<td>${(row.worker.full_name||'').replace(/</g,'&lt;')}</td>`;
      let tot = 0;
      for (const d of days){
        const cell = row.days[d]; const val = cell? cell.hours : 0;
        tot += val;
        const td = document.createElement('td');
        const input = document.createElement('input'); input.type='number'; input.min='0'; input.max='24'; input.step='0.5'; input.value=val;
        input.addEventListener('change', async ()=>{
          await fetch('/.netlify/functions/workers/log/'+cell.id, { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ hours: parseFloat(input.value||'0') }) });
        });
        td.appendChild(input); tr.appendChild(td);
      }
      const totalTd = document.createElement('td'); totalTd.textContent = String(tot);
      tr.appendChild(totalTd); tbody.appendChild(tr);
    }
    table.appendChild(tbody); list.innerHTML=''; list.appendChild(wrap);
  }
}


/* ===========================
   PRECIOS: Comparar y Ver Precio
   =========================== */
(function(){
  let pivot = null; // { suppliers: [{id,name}], items: [{id,name, prices: { [supplierId]: { price, discount, function, brand, description } }}] }
  async function loadPivot(type){
    try{
      const res = await fetch('/.netlify/functions/prices/pivot?type='+encodeURIComponent(type||'tool'));
      if(!res.ok) throw new Error('pivot');
      pivot = await res.json();
    }catch(e){
      console.error('No se pudo cargar pivot', e);
      pivot = { suppliers: [], items: [] };
    }
  }
  async function loadSuppliers(){
    try{
      const res = await fetch('/.netlify/functions/prices/suppliers');
  ...
  function renderCompare(query){
    const box = document.getElementById('cmpResults'); if(!box) return;
    const q = (query||'').toLowerCase().trim();
    if(!pivot || !pivot.items || !pivot.suppliers) { box.innerHTML = '<div class="muted">Sin datos</div>'; return; }
    const items = pivot.items.filter(it => !q || (it.name||it.nombre||'').toLowerCase().includes(q));
    if(items.length===0){ box.innerHTML = '<div class="muted">Sin resultados</div>'; return; }
    const supById = new Map(pivot.suppliers.map(s=>[String(s.id), s]));
    const rows = [];
    for(const it of items){
      const name = it.name || it.nombre || '(sin nombre)';
      const prices = it.prices || it.precios || {};
      for(const [sid, cell] of Object.entries(prices)){
        const sup = supById.get(String(sid));
        if(!sup) continue;
        const price = (cell && (cell.price ?? cell.precio)) ?? '';
        const disc  = (cell && (cell.discount ?? cell.descuento)) ?? '';
        rows.push(`<div class="list-compact">${name} — ${sup.name} — ${price} — ${disc}</div>`);
      }
    }
    box.innerHTML = rows.join('') || '<div class="muted">Sin coincidencias</div>';
  }

  function renderViewPrice(supplierId, query){
    const box = document.getElementById('vpResults'); if(!box) return;
    if(!pivot) { box.innerHTML = '<div class="muted">Sin datos</div>'; return; }
    const q = (query||'').toLowerCase().trim();
    const sid = String(supplierId||'');
    const rows = [];
    for(const it of (pivot.items||[])){
      const name = it.name || it.nombre || '';
      const meta = (it.prices||it.precios||{})[sid];
      if(!meta) continue;
      const brand = meta.brand || meta.marca || '';
      const func  = meta.function || meta.funcion || '';
      const desc  = meta.description || meta.descripcion || '';
      const price = (meta.price ?? meta.precio) ?? '';
      const disc  = (meta.discount ?? meta.descuento) ?? '';
      const hay = [name, brand, func, String(disc)].join(' ').toLowerCase();
      if(q && !hay.includes(q)) continue;
      rows.push(`<div class="list-compact">${name} — ${price} — ${disc} — ${func} — ${brand} — ${desc}</div>`);
    }
    box.innerHTML = rows.join('') || '<div class="muted">Sin datos para este proveedor</div>';
  }

  async function initPrecios(){
    const cmpTipo   = document.getElementById('cmpTipo');
    const cmpSearch = document.getElementById('cmpSearch');
    const vpSupplier= document.getElementById('vpSupplier');
    const vpSearch  = document.getElementById('vpSearch');
    if(!cmpTipo || !vpSupplier) return;

    await loadPivot(cmpTipo.value);
    const sups = await loadSuppliers();
    vpSupplier.innerHTML = sups.map(s=>`<option value="${s.id}">${s.name}</option>`).join('');

    // Events
    cmpTipo.addEventListener('change', async ()=>{
      await loadPivot(cmpTipo.value);
      renderCompare(cmpSearch.value);
      renderViewPrice(vpSupplier.value, vpSearch.value);
    });
    cmpSearch.addEventListener('input', ()=> renderCompare(cmpSearch.value));

    vpSupplier.addEventListener('change', ()=> renderViewPrice(vpSupplier.value, vpSearch.value));
    vpSearch.addEventListener('input', ()=> renderViewPrice(vpSupplier.value, vpSearch.value));

    // Initial paint
    renderCompare('');
    if (vpSupplier.value) renderViewPrice(vpSupplier.value, '');
  }

  // Initialize on route enter
  window.addEventListener('hashchange', ()=>{
    if (location.hash.replace('#','') === 'precios') initPrecios();
  });
  if (location.hash.replace('#','') === 'precios') initPrecios();
})();
